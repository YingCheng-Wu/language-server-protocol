...
const trim = (input) => input.replace(/^\s+/, '').replace(/\s+$/, ''), spinnerdiv = dgEBI('spinnerdiv'), entry = dgEBI('entry'), sampletextarea = dgEBI('sampletextarea'), resulttextarea = dgEBI('resulttextarea'), contentlength = dgEBI('contentlength'), invalidjson = dgEBI('invalidjson'), tableofcontents = dgEBI('tableofcontents'), listitem = dgEBI('listitem'), startrequest = dgEBI('startrequest'), hintdiv = dgEBI('hintdiv'), methodtypeh3 = dgEBI('methodtypeh3'), methodstringh3 = dgEBI('methodstringh3'), editmode = dgEBI('editmode'), codebeautify = dgEBI('codebeautify'), resultpre = dgEBI('resultpre'), contentlengthspan = dgEBI('contentlengthspan'), editsyntaxpre = dgEBI('editsyntaxpre'), resultlengthdiv = dgEBI('resultlengthdiv'), resultlengthspan = dgEBI('resultlengthspan'), resultbeautify = dgEBI('resultbeautify'), resulttabs = dgEBI('resulttabs'), resultdiv = dgEBI('resultdiv');
...
function hideSpinner() {
    ...
    for (const animation of spinnerdiv.getAnimations()) {
        animation.cancel();
    }
}
...
function showEntry() {
    const li = this.parentNode, methodstring = gEBTN0(li, 'a').innerText;
    entry.showModal();
    ...
    if (contentlength.hidden) {
        ...
        if (startRange < 3 && startRange > 0) {
            ...
            if (selectionStart >= startIndex) {
                ...
                sampletextarea.selectionStart = 0;
            }
        }
    }
...
class showHintdiv {
    static showPlayButtonHint() { showHintdiv.Hintdiv.showHint(this, 'Try it out'); }
    static showEditButtonHint() { showHintdiv.Hintdiv.showHint(this, 'Edit request'); }
    static showDocIconHint() { showHintdiv.Hintdiv.showHint(this, 'Show specification'); }
    static showSpecIconHint() { showHintdiv.Hintdiv.showHint(this, 'Open documentation'); }
    static showMoreIconHint() { showHintdiv.Hintdiv.showHint(this, 'Look for the definition'); }
    static showPlayButtonInDeTeHint() { showHintdiv.Hintdiv.showHint(this, 'Try it out', { direction: 'left' }); }
    static showSaveButtonHint() { showHintdiv.Hintdiv.showHint(this, 'Save', { direction: 'left' }); }
    static showMethodMessageHint() { showHintdiv.Hintdiv.showHint(this, DefaultValues.methodMessages[DefaultValues.methodTypeIndex[this.nodeName.toLowerCase()]]); }
    static updateDefaultValues() { DefaultValues.updateDefaultValues(this).then(response => { if (response.status === 200) {
        showHintdiv.Hintdiv.showHint(this, 'Okay', { direction: 'left' });
    } }); }
}
...
fetch(RemoteURL.remoteURL + 'exampleMethods.txt').then(v => v.text()).then(values => {
    ...
    for (const li of gEBTN(tableofcontents, 'li')) {
        const container = listitem.content.cloneNode(true), children = container.children, [playButton, editButton, docIcon, specIcon, moreIcon, defaultTextarea] = children, [, playButtonInDeTe, saveButton] = defaultTextarea.children, click = 'click', hover = 'pointerenter';
        on(playButton, click, showEntry);
        on(playButton, hover, showHintdiv.showPlayButtonHint);
        on(editButton, click, toggleTextarea);
        on(editButton, hover, showHintdiv.showEditButtonHint);
        on(docIcon, click, OpenWebPage.openDocumentPage);
        on(docIcon, hover, showHintdiv.showDocIconHint);
        on(specIcon, click, OpenWebPage.openSpecificationPage);
        on(specIcon, hover, showHintdiv.showSpecIconHint);
        on(moreIcon, click, OpenWebPage.openDocumentationPage);
        on(moreIcon, hover, showHintdiv.showMoreIconHint);
        on(playButtonInDeTe, click, showEntryFromChild);
        on(playButtonInDeTe, hover, showHintdiv.showPlayButtonInDeTeHint);
        on(saveButton, click, showHintdiv.updateDefaultValues);
        on(saveButton, hover, showHintdiv.showSaveButtonHint);
        const methodString = gEBTN0(li, 'a').innerText, methodElement = li.firstElementChild, methodTypeString = DefaultValues.methodTypeIndex[DefaultValues.methodIconIndex[methodElement.getAttribute('alias')]], methodTypeElement = document.createElement(methodTypeString);
        methodElement.replaceWith(methodTypeElement);
        on(methodTypeElement, hover, showHintdiv.showMethodMessageHint);
        container.lastElementChild.firstElementChild.value = methodString && DefaultValues.defaultValues[methodString] || '{"jsonrpc":"2.0",' + (methodTypeString.split('-')[1] == 'request' ? '"id":0,' : '') + '"method":"' + methodString + '","params":{}}';
